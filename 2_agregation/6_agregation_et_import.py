...
def import_to_database(engine, fact_trips: pd.DataFrame, fact_alerts: pd.DataFrame, dim_iris: pd.DataFrame):
    ...
    # Erreur réseau ou panne du conteneur PostgreSQL pendant le transfert
    except OperationalError as e:
        print(f"Erreur d'infrastructure : Perte de connexion ou timeout lors de l'import : {e}")

    # Incompatibilité entre le schéma inféré par Pandas et les contraintes PostgreSQL
    except DataError as e:
        print(f"Erreur d'intégrité : Incompatibilité de type ou donnée hors limites détectée par PostgreSQL : {e}")

    # Erreur interne à Pandas (ex: DataFrame mal formaté avant l'envoi)
    except ValueError as e:
        print(f"Erreur Pandas : Impossible de préparer l'importation SQL : {e}")
...
if __name__ == "__main__":
    ...
    # Le schéma des données a changé (ex: la colonne 'telephone_emetteur' a été renommée à la source)
    except KeyError as e:
        print(f"Arrêt critique (C3) : Colonne manquante lors du nettoyage ou de l'agrégation : {e}")
        sys.exit(1)

    # La donnée est corrompue (ex: impossible de convertir une date, ou types incompatibles)
    except ValueError as e:
        print(f"Arrêt critique (C3) : Problème de typage ou de format de donnée : {e}")
        sys.exit(1)

    # L'infrastructure est tombée (ex: get_db_engine() n'a pas pu se connecter à PostgreSQL)
    except SQLAlchemyError as e:
        print(f"Arrêt critique (C4) : Échec de l'infrastructure de base de données : {e}")
        sys.exit(1)
