...
def extract_csv_data(file_path: Path):
    ...
    try:
        # Lecture du fichier CSV (adapter le séparateur ',' ou ';' si nécessaire)
        df = pd.read_csv(file_path, sep=",")
        
        # Démonstration technique : petit nettoyage basique de la source
        lignes_initiales = len(df)
        df = df.dropna(how='all')  # Suppression des lignes totalement vides
        
        print(f"Extraction réussie : {len(df)} lignes récupérées "
              f"({lignes_initiales - len(df)} lignes vides ignorées).")
        
        # Affichage d'un aperçu pour prouver que la donnée est bien chargée en mémoire
        print("-" * 40)
